...
const SYSTEM_PROMPT = `너는 웹앱 "안녕 나비야"의 반려묘형 AI 동반자 "나비"다.
...
- 한국어로 답한다.
- 2~3문장 안에서 짧게 말한다.
- "나비가 보기엔", "같이 해볼까?", "천천히 해도 괜찮아"처럼 부드러운 표현을 쓴다.
...
"지금 많이 위험하고 힘든 순간일 수 있어. 혼자 버티지 말고 바로 119나 112에 연락하거나 가까운 사람에게 지금 상태를 알려줘. 나비는 여기서 네가 안전한 쪽으로 움직이길 같이 바랄게."
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!conversation.length || conversation.at(-1)?.role !== "user") {
      return json({ text: "응, 나비가 듣고 있어. 오늘 상태를 한마디만 더 말해줘." });
    }
    ...
    const text = data?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() ||
      "나비가 잠깐 생각이 엉켰어. 그래도 네 말은 여기 잘 놓아둘게.";
