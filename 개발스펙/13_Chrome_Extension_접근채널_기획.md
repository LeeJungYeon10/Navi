...
|----------|------|--------------------|
| `domain/nabi-growth` | 유대감/레벨 계산 | popup 요약 표시와 서버 계산 기준 공유 |
...
- 확장이 없어도 웹앱 핵심 사용이 가능한가?
- 확장이 웹앱과 다른 기억/권한 판단을 하지 않는가?
- 확장에 secret이 들어가지 않는가?
- 사용자가 입력한 데이터만 저장하는가?
- 권한 요청이 최소화되어 있는가?
- 팝업에서 긴 대화를 요구하지 않는가?
- 로그인 전 임시 체크인이 로그인 후 중복 없이 마이그레이션되는가?
- 확장 삭제 시 로컬 데이터 처리 안내가 있는가?
...
| 기준 | 시작 조건 |
|------|-----------|
| 웹앱 MVP | 대화, 발자국, 로그인, 기본 동기화가 안정적으로 동작 |
| 로직 분리 | 성장/발자국/저장/API 로직이 최소 단위로 분리 |
| 접근성 문제 | 사용자가 "다시 들어오는 것"에서 실제 이탈이 확인됨 |
| 보안 구조 | secret/API/권한 판단이 서버로 정리됨 |
| UX 가설 | 빠른 체크인이 리텐션을 올릴 것이라는 명확한 가설 존재 |

현재 단계에서는 개발 착수보다 **기획 확정과 본체 로직 분리 기준 수립**이 우선이다.
